...
export default function Login() { 
  ...
  return (
  <>
  {global.getlocalStorage("emailId") && global.getlocalStorage("password")?<Navigate to={"/loggedIn/search"}/>:
 <form className='loginForms'>
 <h2 className="text-center p-2">Welcome To Gaurdians</h2>
 <div className=" d-flex form-outline mb-4">
  <input type="email" id="form2Example1" className="form-control" value={email} onChange={(e)=>{setemail(e.target.value)}} placeholder="Email" />
  </div>
  ...
  <button type="button" onClick={submit} className="btn btn-primary btn-block mb-4 w-100">Sign in</button>
